...
public class AutoCutterScreen extends BaseAutoScreen<AutoCutterScreenHandler> {
    private static final Identifier TEXTURE = new Identifier(ServerMod.modId, "textures/gui/container/autocutter.png");
    private static final Identifier TEXTURE_VANILLA = new Identifier("textures/gui/container/stonecutter.png");
    ...
    protected void drawBackground(DrawContext matrices, float delta, int mouseX, int mouseY) {
        ...
        matrices.drawTexture(TEXTURE_VANILLA, x+119, y+RECIPE_LIST_OFFSET_Y+(int)(41f*this.scrollAmount), 176, 0, SCROLLBAR_WIDTH, SCROLLBAR_HEIGHT);
        ...
        for(int i = this.scrollOffset; i < RECIPE_LIST_COLUMNS*RECIPE_LIST_ROWS && i < handler.getAvailableRecipeCount(); i++) {
            ...
            int v =
                i == selectedRecipeIndex? 184:
                mouseX >= iX && mouseY >= iY && mouseX < iX + RECIPE_ENTRY_WIDTH && mouseY < iY + RECIPE_ENTRY_HEIGHT? 202:
                166;
            matrices.drawTexture(TEXTURE_VANILLA, iX, iY-1, 0, v, RECIPE_ENTRY_WIDTH, RECIPE_ENTRY_HEIGHT);
            matrices.drawItem(handler.getAvailableRecipes().get(i).getOutput(registry), iX, iY);
        }
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        ...
        this.scrollAmount = MathHelper.clamp(this.scrollAmount - (float)amount / (float)i, 0F, 1F);
