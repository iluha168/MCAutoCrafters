...
public class AutoCutterBlockEntity extends BaseAutoBlockEntity {
	...
	public static List<StonecuttingRecipe> getAvailableRecipes(Inventory input, World world){
		return world.getRecipeManager().getAllMatches(RecipeType.STONECUTTING, input, world);
	}

	public static ItemStack craftStatic(Inventory input, World world, List<StonecuttingRecipe> availableRecipes, int recipeIndex){
        return (recipeIndex >= 0 && recipeIndex < availableRecipes.size())?
            availableRecipes.get(recipeIndex).craft(input, world.getRegistryManager())
            : ItemStack.EMPTY;
	}
